        ...
        void DepartmentFreeTextSearch()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(txtFreeTextSearch.Text.Trim()))
                {
                    ...
                    Response.Redirect("/m/Departments/Search-Result?lng=" + EncryptDecryptHelper.Encrypt(txtFreeTextSearch.Text).ToString());
                    ...
                    if (dt != null)
                    {
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            Session["DeptSearch"] = dt;
                            Session["DeptSearchKeyWord"] = txtFreeTextSearch.Text;
                            Response.Redirect("/m/Departments/Search-Result?lng=" + EncryptDecryptHelper.Encrypt(txtFreeTextSearch.Text));
                        }
                        else
                        {
                            lblMsg.Text = "No Records Found !";
                ...
                if (!string.IsNullOrEmpty(txtSearchByDate.Text.Trim()))
                {
                    ...
                    if (dt != null)
                    {
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            Session["DeptSearch"] = dt;
                            Session["DeptSearchKeyWord"] = txtSearchByDate.Text;
                            Response.Redirect("/m/Departments/Search-Result?lng=" + EncryptDecryptHelper.Encrypt(txtSearchByDate.Text));
                        }
                        else
                        {
                            lblMsg.Text = "No Records Found !";
