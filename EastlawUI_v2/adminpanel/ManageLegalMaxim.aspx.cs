...
{
    public partial class ManageLegalMaxim : System.Web.UI.Page
    {
        EastLawBL.LegalMaxim objlm = new EastLawBL.LegalMaxim();
        ...
        public void GetLegalMaxim(int ID)
        {
            try
            {

                DataTable dt = objlm.GetLegalMaximWord(ID);
                if (dt.Rows.Count > 0)
                {
        ...
        protected void grdContact_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            objlm.Word = txtEditWord.Text.Trim();
            objlm.Meaning = txtEditMeaning.Text.Trim();
            if (chkActive.Checked == true)
                objlm.Active = 1;
            else
                objlm.Active = 0;
            objlm.ModifiedBy = int.Parse(Session["UserID"].ToString());
            int chk = objlm.EditLegalMaximWord(int.Parse(hdID.Value.ToString()));
        ...
        protected void grdContact_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                ...
                if ((row != null))
                {
                    ...
                    int ID = Convert.ToInt32(grdContact.DataKeys[e.RowIndex].Values[0].ToString());
                    int chk = objlm.DeleteLegalMaximWord(ID, int.Parse(Session["UserID"].ToString()));
                    if (chk > 0)
                    {
        ...
        protected void grdContact_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName.Equals("Insert"))
            {
                ...
                objlm.Word = txtNewWord.Text.Trim();
                objlm.Meaning = txtNewMeaning.Text.Trim();
                if (chkNewActive.Checked == true)
                    objlm.Active = 1;
                else
                    objlm.Active = 0;
                objlm.CreatedBy = int.Parse(Session["UserID"].ToString());
                int chk = objlm.InsertLegalMaximWord();
                if (chk > 0)
                {
                    divSuccess.Style["Display"] = "";
                    divError.Style["Display"] = "none";
