    ...
    public partial class ManageUsers : System.Web.UI.Page
    {
        ...
        void GetActiveUserTypes()
        {
            try
            {
                ...
                ddlUserType.Items.Insert(0, new ListItem("All", "0"));
            }
            catch (Exception ex)
            {
        ...
        void UsersSearch()
        {
            try
            {
                string cri = "Select A.*,DATEDIFF(DAY, getdate(),CONVERT(datetime,A.AccessExpireOn)) as ExpireIn,CONVERT(datetime,A.AccessExpireOn) as FormatedExpire,E.UserType,B.Name as CountryName,C.PlanName,C.NoofDays,D.CompanyName"
            ...
            + " inner join dbo.tbl_UserType E on A.UserTypeID=E.ID Where A.CreatedOn is not null and A.isdeleted=0";
                ...
                cri=cri +" union all "
            + " Select A.*,DATEDIFF(DAY, getdate(),CONVERT(datetime,A.AccessExpireOn)) as ExpireIn,CONVERT(datetime,A.AccessExpireOn),D.UserType,B.Name as CountryName,C.PlanName,C.NoofDays,'' as CompanyName" 
            + ",(select top 1 convert(varchar(101), Createdon)  from tbl_auditlog where activitytype='Login/Logout' and [Action] like '%Success%' and userid=A.ID) as FirstLogin " 
            + " ,'' as FirstLogin "
            + " from dbo.tbl_Users A"
		    +" inner join Country B on A.Country=B.Code"
