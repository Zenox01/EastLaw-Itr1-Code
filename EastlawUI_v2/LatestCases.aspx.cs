...
{
    public partial class LatestCases : System.Web.UI.Page
    {
        ...
        void BindSearchResult()
        {
            try
            {
                ...
                if (HttpContext.Current.Items["CaseFilter"] != null)
                {
                    CaseFilter = HttpContext.Current.Items["CaseFilter"].ToString();
                }
                lblCurCrumb.InnerText = CommonClass.MakeFirstCap(CaseFilter.Replace("-", " "));
                lblSearchWords.Text = CommonClass.MakeFirstCap(CaseFilter.Replace("-", " "));
                if (CaseFilter == "latest-judgments")
                {
                    InsertAuditLog("Cases", "Update Panel", CaseFilter);
                    dt = objcase.GetLatestCases();
                }
                ...
                else if (CaseFilter == "high-court")
                {
                    InsertAuditLog("Cases", "Update Panel", CaseFilter);
                    dt = objcase.GetCasesByCourt("high court");
                }
                else if (CaseFilter == "practice-area-latest-judgments")
                {
                    EastLawBL.PracticeAreas objPA1 = new EastLawBL.PracticeAreas();
                    lblCurCrumb.InnerText = CommonClass.MakeFirstCap(CaseFilter.Replace("-", " ") + " - " + Request.QueryString["trans"].ToString().Replace("-", " "));
                    lblSearchWords.Text = CommonClass.MakeFirstCap(CaseFilter.Replace("-", " ") + " - " + Request.QueryString["trans"].ToString().Replace("-", " "));
                    InsertAuditLog("Cases", "Update Panel", lblSearchWords.Text);
                    int PracticeAreaID = int.Parse(EncryptDecryptHelper.Decrypt(Request.QueryString["param"].ToString()).ToString());
                    dt = objPA1.GetTaggesCasesWithPracticeArea(PracticeAreaID);
                ...
                var queryYear = from row in dt.AsEnumerable()
                                ...
                                group row by row.Field<string>("Month") into YEARS
                                orderby YEARS.Key
                ...
                chkLstYear.DataSource = queryYear;// CreateYearRange(dt);
        ...
        void FilterResults()
        {
            try
            {
                ...
                dtFilter = (DataTable)Session["LatestCases"];
